...
def generateLegalMoves(index, piece, captures):
    ...
    if piece[0] == "K":
        ...
        moves = kingMove(index, blackLegalMoves(board)) + index
    
    if piece[0] == "k":
        moves = kingMove(index, whiteLegalMoves(board)) + index
...
def kingInCheck(move, currentPos):
    ...
    if turnPlayer == "w" and whiteKingPos not in blackLegalMoves(newBoard):
        return False
    elif turnPlayer == "b" and blackKingPos not in whiteLegalMoves(newBoard):
        return False
    else:
        return True
            ...
            if selectedMoves:
                for count in range(int((len(moves) - 2)/2)):
                    if int(moves[count * 2]) == cellX and int(moves[count * 2 + 1]) == cellY:
                        ...
                        if board[cellY][cellX][0] == "K":
                            whiteKingPos = str(cellX) + str(cellY)
                        elif board[cellY][cellX][0] == "k":
                            blackKingPos = str(cellX) + str(cellY)
