...
def generateLegalMoves(index, piece, captures):
    ...
    if piece[0] == "P":
        ...
        if not captures and index[1] == "6" and board[int(index[1]) - 2][int(index[0])][0] == "":
        ...
        if captures and int(index[0]) > 0 and board[int(index[1]) - 1][int(index[0]) - 1][1] != "w":
           moves = moves + str(int(index[0]) - 1) + str(int(index[1]) - 1)
        if captures and int(index[0]) < 7 and board[int(index[1]) - 1][int(index[0]) + 1][1] != "w":
            moves = moves + str(int(index[0]) + 1) + str(int(index[1]) - 1)
    ...
    if piece[0] == "K":
        ...
        for i in range(8):
            for j in range(8):
                if board[i][j][1] == "b" and board[i][j][0] != "k":
                    move = generateLegalMoves(str(j) + str(i), board[i][j][0], True)
                    print (move)
                    for count in range(int((len(move) - 2)/2)):
                        legalMoves.append(move[count * 2] + move[count * 2 + 1])
        print (legalMoves)
#        for count in range(len(legalMoves)):
#            pygame.draw.rect(gameWindow, (0, 0, 255), (int(legalMoves[count][0]) * 100, int(legalMoves[count][1]) * 100, 100, 100), 0)
#        drawPieces(board)
        turnPlayer = "w"
    ...
    if piece[0] == "k":
        ...
        for i in range(8):
            for j in range(8):
                if board[i][j][1] == "w" and board[i][j][0] != "K":
                    move = generateLegalMoves(str(j) + str(i), board[i][j][0], True)
                    for count in range(int((len(move) - 2)/2)):
                        legalMoves.append(move[count * 2] + move[count * 2 + 1])
        turnPlayer = "b"
    ...
    if piece[0] == "p":
        if not captures and board[int(index[1]) + 1][int(index[0])][0] == "":
            moves = moves + index[0] + str(int(index[1]) + 1)
        if not captures and index[1] == "1" and board[int(index[1]) + 2][int(index[0])][0] == "":
            moves = moves + index[0] + str(int(index[1]) + 2)
        ...
        if captures and int(index[0]) > 0 and board[int(index[1]) + 1][int(index[0]) - 1][1] != "b":
           moves = moves + str(int(index[0]) - 1) + str(int(index[1]) + 1)
        if captures and int(index[0]) < 7 and board[int(index[1]) + 1][int(index[0]) + 1][1] != "b":
            moves = moves + str(int(index[0]) + 1) + str(int(index[1]) + 1)
