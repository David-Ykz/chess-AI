...
abstract class Pieces implements Cloneable {
  ...
  public int findKingPos(Board board) {
    ...
    System.out.println("findKingPos, Pieces");
    return -1;
  }
  ...
  public ArrayList<Integer> findLegalMoves(Board board){
    ...
    for (int i = 0; i < possibleMoves.size(); i++) {
      if (board.friendlySquare(possibleMoves.get(i), this.color) == false) {
        board.movePiece(this, possibleMoves.get(i));
        if (findAllMoves(board, this.color).contains(kingPos) == false) {
          legalMoves.add(this.position);
        }
        board.setPieces(savePieces);
        this.position = saveCurrentPos;
